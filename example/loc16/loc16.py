...
for i in range(int(np.floor(X.shape[0] / batch_size))):
    ...
    for model in range(len(models)):
    	dill.dump(models[model], open("model_%d_loc16.dill" % (model), "wb"))
...
plt.savefig("loc_16.png", bbox_inches='tight', dpi = 600)
...
df.to_csv("rewards_loc16.csv", index=False)